...
import toHTMLWithData = Blaze.toHTMLWithData;
    ...
    reply: function(partyId, rsvp) {
        ...
        if (rsvpIndex !== -1) {
            ...
            if (Meteor.isServer) {
                ...
                Parties.update(
                    { _id: partyId, 'rsvp.userId': this.userId },
                    { $set: { 'rsvp.$.response': rsvp } }
                );
            } else {
                // minimongo doesn't yet support $ in modified, as a temporary
                // workaround, make a modified that uses an index. this is
